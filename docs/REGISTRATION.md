...
**Profile complete:** `staffProfile.isProfileComplete` is `true` when `profilePicture`, `stageName`, and `bio` are all set.
...
- **STAFF** — `profilePicture` required + `staffProfile` object required
...
| Date | Change |
|------|--------|
| 2026-06-29 | Initial registration spec with FileMeta upload flow |
